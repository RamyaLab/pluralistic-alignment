...
from .utils import calc_hinge_loss, calc_logisitic_loss
from ..llms_dataset import preprocess_tokenized_ds
...
class LearnerWrapLightning(L.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if "new_pair" in self.learner_mode:
            ...
            self.log("TestAccuLastToken", accu_last_token, on_step=True, on_epoch=True, prog_bar=True, logger=True)
            self.log("TestAccuMeanToken", accu_mean_token, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        elif "new_user" in self.learner_mode:
            self.log("new_TestLoss", loss, on_epoch=True, prog_bar=False, logger=True)
            self.log("new_TestAccuLastToken", accu_last_token, on_step=True, on_epoch=True, prog_bar=True, logger=True)
            self.log("new_TestAccuMeanToken", accu_mean_token, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        return loss
