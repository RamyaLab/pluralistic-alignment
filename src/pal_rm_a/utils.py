...
def calc_logisitic_loss(y_hat, y, loss_fn, inds, weightingMethod: Literal['mean', 'cumulative', 'only_last']='mean'):
    '''
    this function calculate the pairwise loss given the predicted reward score and the true choice
    ...
    for i in range(bs):
        ...
        if weightingMethod == 'mean':
            ...
            loss += calc_only_last(tmp_loss)
        accu_last_token += (y_hat_i[-1,:].argmax(dim=1) == y[i]).float().mean().item()
        accu += (y_hat_i.argmax(dim=1) == y_truncated).float().mean().item()
    accu /= bs
    loss /= bs
    accu_last_token /= bs
    return {'loss':loss, 'accu':accu, 'accu_last_token':accu_last_token}
