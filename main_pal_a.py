...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_name', type=str, default='summary-a')
    ...
    wandb_logger = WandbLogger(project='NEW-pal-rm-a-seen-opt350m-bound-loss', name=args.run_name)
    ...
    # learner.prefLearner.update_trainable_params(fix_modules=["llm"])
    learner.prefLearner.update_trainable_params()
    learner.set_mode("new_pair")
    trainer = L.Trainer(max_epochs=optim_config.epochs_new_pair, logger=wandb_logger, callbacks=[checkpoint_callback])
    if trainer.global_rank == 0:
        wandb_logger.experiment.config.update(config_dict)

    # check how many trainable parameters are there in the learner
    # Train
